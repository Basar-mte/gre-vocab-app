...
## Gotchas hit while deploying (for future reference)

- **`middleware.ts` → `proxy.ts`**: Next.js 16 renamed the convention, and critically, **Proxy defaults to the Node.js runtime** where `middleware.ts` defaulted to Edge. The auth proxy (`src/proxy.ts`) imports the full `@/auth` (Credentials provider + bcryptjs + Prisma) directly — on Edge that bundle exceeded Vercel's 1 MB Edge Function limit and failed every deploy at the "Deploying outputs" step; on the Node runtime there's no such limit, so there's no need to split the NextAuth config into an edge-only variant.
- **No BOM in env var values.** Setting `NEXTAUTH_URL`/`AUTH_SECRET` via `"value" | vercel env add NAME production` from PowerShell silently prepended a UTF-8 BOM (`﻿`) to the stored value. NextAuth's internal `new URL(process.env.NEXTAUTH_URL)` then threw `TypeError: Invalid URL` on every single request in production (visible via `vercel logs <deployment-url>`, not in the build log). Fix: pipe env var values through `printf '%s' "value" | vercel env add NAME production` (bash) instead — verify with `printf '%s' "value" | xxd | head -1` that the first bytes are the expected ASCII, not `ef bb bf`.
- **Vercel Deployment Protection ("Vercel Authentication")** is on by default for new projects and gates *every* domain (including a manually-aliased `something.vercel.app`) behind a Vercel SSO redirect — a logged-out visitor gets bounced to `vercel.com/sso-api` instead of your app. Disable it under Project Settings → Deployment Protection (or via `vercel api "/v9/projects/<id>?teamId=<team>" -X PATCH --input <(echo '{"ssoProtection": null}')`).
- **A manual `vercel alias set <deployment> <domain>` pins to that one deployment** — it does not automatically follow future production deploys the way the project's default `<name>.vercel.app` domain does. After every deploy the alias needs re-pointing to the new deployment URL (`vercel ls <project>` to find it), unless the domain is instead attached through the project's Domains settings for auto-follow.
